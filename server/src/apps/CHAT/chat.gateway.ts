import { forwardRef, Inject, Logger, UseFilters, UseGuards, UsePipes, ValidationPipe } from "@nestjs/common";
...
export class ChatGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
    ...
    async handleDisconnect(client: SocketWithAuth) {
        ...
        this.io.to(client.roomsId).emit(ChatEvent.NewMemberBroadCast, { onlineUsers: uniqueOnlineUsers })
    ...
    async getUniqueOnlineUsers(client: SocketWithAuth) {
        ...
        const uniqueOnlineUsers = (await Promise.all(
            ...
            .reduce((p, c: any[], i) => ({
                ...p,
                [allRooms[i]]:
                    getArrayOfObjectUniqulyByKey(c.map(item => item.user).filter(x => x !== undefined), 'id')
    ...
    async NewMemberEvent(@ConnectedSocket() client: SocketWithAuth) {
        ...
        const addDeliveryToMyAllMessages = await this.chatService.addDeliveredToMyAllMessages(client.user)
        if (addDeliveryToMyAllMessages.length > 0) {
            this.io.to(client.roomsId)
                .emit(ChatEvent.MultipleDeliveringBroadCast, {
    ...
    async ChangeActiveRoomEvent(
        ...
        const addSeenToAllMessagesOfThisActiveRoom = await this.chatService.addSeenToThisRoomAllMessages(client.user, currentActiveRoomId)

        if (addSeenToAllMessagesOfThisActiveRoom.length > 0) {
            this.io.to(currentActiveRoomId.toString())
    ...
    async NewMessageEvent(
        ...
        const uniqueOnlineUsers = await this.getUniqueOnlineUsers(client)
        await this.chatService.addMessageDelivering(client.user, msg, uniqueOnlineUsers)
        const message = await this.chatService.getMessage(client.user, msg.id)
        this.io.to(message.room.id.toString()).emit(ChatEvent.NewMessageBroadCast, message)
    }
    ...
    async NewMessageSeenEvent(
        ...
        const addSeen = await this.chatService.addMessageSeen(client.user, messageId, roomId)
        if (addSeen) {
            const membersId = (await this.chatService.getRoomMembers(roomId)).map(item => item.id).sort()
    ...
    async NewRoomCreatedEvent(
        ...
        const intendedParticipants = thisRoom.intendedParticipants.filter(item => item.status === chatIntendedParticipantStatus.requested)

        client.roomsId.push(thisRoom.id.toString())
        await client.join(thisRoom.id.toString())

        this.io.emit(ChatEvent.NewRoomIntendedParticipantBroadcast, intendedParticipants)

        this.io.to(thisRoom.id.toString()).emit(ChatEvent.NewRoomCreatedBroadcast, thisRoom)
    ...
    async JoinRequestConfirmationEvent(
        ...
        const thisIntendedParticipant = await this.chatService.findIntendedParticipantById(chatIntendedParticipant.id)
