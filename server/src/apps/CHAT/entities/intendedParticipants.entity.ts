...
export class ChatIntendedParticipants {
  ...
  @Column({
    type: 'enum',
    enum: chatIntendedParticipantStatus,
    default: chatIntendedParticipantStatus.requested,
  })
  ...
  @ManyToOne(() => User, (user) => user.creatorOfIntendedParticipants, {
    nullable: false,
  })
  creator: User;

  @ManyToOne(() => User, (user) => user.participantOfIntendedParticipants, {
    nullable: false,
  })
  participant: User;
  ...
  @OneToMany(
    ...
    (delieveredMessage) => delieveredMessage.intendedParticipant,
    { cascade: true },
  )
