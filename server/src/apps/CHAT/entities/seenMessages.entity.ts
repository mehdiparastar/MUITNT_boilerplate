...
export class ChatSeenMessages {
    @PrimaryGeneratedColumn()
    @ApiProperty()
    id: number;

    @ManyToOne(() => ChatMessage, (message) => message.delivered, { nullable: false })
    message: ChatMessage;

    @ManyToOne(() => ChatIntendedParticipants, (intendedParticipant) => intendedParticipant.delivered, { nullable: false })
