...
import {
  IChatRoomAddMessageFormDto,
  MessageDto,
} from 'models/CHAT_APP/message.model';
import {
  ICreateChatRoomFormDto,
  RoomDtoWithoutMessages,
} from 'models/CHAT_APP/room.model';
    ...
    >({
      ...
      async onCacheEntryAdded(
        ...
        try {
          ...
          chatSocket.emit(ChatEvent.NewRoomCreated, {
            ...getCacheEntry().data,
          });
    ...
    getMyAllRooms: builder.query<RoomDtoWithoutMessages[], void>({
      ...
      async onCacheEntryAdded(
        ...
        try {
          ...
          chatSocket.on(
            ChatEvent.NewRoomCreatedBroadcast,
            ({
              newRoom,
              socketOwner,
            }: {
              newRoom: RoomDtoWithoutMessages;
              socketOwner: IUser;
            }) => {
              const currentUser = (getState() as RootState).apiSlice.queries[
                'getCurrentUser(undefined)'
              ]?.data as IUser;
              if (currentUser.id === socketOwner.id)
                if (
                  newRoom.intendedParticipants.map(
                    (item) =>
                      item.status === chatIntendedParticipantStatus.accepted,
                  ).length > 1
                )
                  if (
                    getCacheEntry().data?.find(
      ...
      async onCacheEntryAdded(
        ...
        try {
          ...
          chatSocket.on(
            ...
            (newIntendedParticipants: RoomIntendedParticipantDto[]) => {
              const currentUser = (getState() as RootState).apiSlice.queries[
                'getCurrentUser(undefined)'
              ]?.data as IUser;
              const acceptableIntendedParticipant =
                newIntendedParticipants.filter(
                  (item) => item.participant.id === currentUser.id,
                );
              if (acceptableIntendedParticipant.length > 0)
                for (const newIntendedParticipant of acceptableIntendedParticipant)
                  if (
                    getCacheEntry().data?.find(
                      (intendedParticipant) =>
                        intendedParticipant.id === newIntendedParticipant.id,
                    ) === undefined
                  ) {
                    const update = updateCachedData((draft) => {
                      draft.push({
          ...
          chatSocket.on(
            ChatEvent.ConfirmRequestBroadcast,
            (confimedIntendedParticipant: RoomIntendedParticipantDto) => {
              const currentUser = (getState() as RootState).apiSlice.queries[
                'getCurrentUser(undefined)'
              ]?.data as IUser;

              if (confimedIntendedParticipant.participant.id === currentUser.id)
                if (
                  getCacheEntry()
                    .data?.map((intendedParticipant) => intendedParticipant.id)
                    .includes(confimedIntendedParticipant.id)
                ) {
                  const update = updateCachedData((draft) => {
                    return draft.filter(
                      (item) => item.id !== confimedIntendedParticipant.id,
                    );
                  });
          ...
          chatSocket.on(
            ChatEvent.RejectRequestBroadcast,
            (confimedIntendedParticipant: RoomIntendedParticipantDto) => {
              const currentUser = (getState() as RootState).apiSlice.queries[
                'getCurrentUser(undefined)'
              ]?.data as IUser;
    ...
    confirmJoinRequest: builder.mutation<RoomIntendedParticipantDto, number>({
      ...
      async onCacheEntryAdded(
        ...
        try {
          ...
          chatSocket.emit(ChatEvent.JoinRequestConfirmed, {
            ...getCacheEntry().data,
          });
    ...
    rejectJoinRequest: builder.mutation<RoomIntendedParticipantDto, number>({
      ...
      async onCacheEntryAdded(
        ...
        try {
          ...
          chatSocket.emit(ChatEvent.JoinRequestRejected, {
            ...getCacheEntry().data,
          });
    ...
    addMessage: builder.mutation<MessageDto, IChatRoomAddMessageFormDto>({
      ...
      async onCacheEntryAdded(
        ...
        try {
          ...
          chatSocket.emit(ChatEvent.NewMessage, {
            ...getCacheEntry().data,
            room: { id: arg.roomId },
          });
        ...
        try {
          ...
          chatSocket.on(ChatEvent.NewMessageBroadCast, (newMsg: MessageDto) => {
            if (arg.roomId === newMsg.room.id)
              if (
                getCacheEntry().data?.find((msg) => msg.id === newMsg.id) ===
                undefined
              ) {
                const update = updateCachedData((draft) => {
                  draft.push({
                ...
                if (
                  arg.roomId.toString() ===
                  localStorage.getItem('active-room-id')?.toString()
                )
                  chatSocket.emit(ChatEvent.MessageSeen, {
                    messageId: newMsg.id,
                    roomId: arg.roomId,
                  });
                return update;
          ...
          chatSocket.on(
            ChatEvent.MessageSeenBroadCast,
            ({
              seenMessageId,
              seenUser,
              membersId,
              ...
              if (roomId === arg.roomId)
                if (
                  !getCacheEntry()
                    .data?.find((msg) => msg.id === seenMessageId)
                    ?.seen.map((item) => item.id)
                    .includes(seenUser.id)
                ) {
                  const update = updateCachedData((draft) => {
                    const foundIndex = draft.findIndex(
                      (item) => item.id === seenMessageId,
                    );
                    draft[foundIndex].seen.push(seenUser);
                    draft[foundIndex].isSeen =
                      JSON.stringify(membersId) ===
                      JSON.stringify(
                        draft[foundIndex].seen.map((item) => item.id).sort(),
                      );
                    return draft;
                  });
                  return update;
          ...
          chatSocket.on(
            ChatEvent.MultipleDeliveringBroadCast,
            ({ messages }: { messages: MessageDto[] }) => {
              ...
              const update = updateCachedData((draft) => {
                return draft.map((item) =>
                  thisRoomMessages[item.id.toString()]
                    ? {
                        ...thisRoomMessages[item.id],
                        createdAt: new Date(
                          thisRoomMessages[item.id].createdAt,
                        ),
                        updatedAt: new Date(
                          thisRoomMessages[item.id].updatedAt,
                        ),
                      }
                    : item,
                );
          ...
          chatSocket.on(
            ChatEvent.MultipleSeenBroadCast,
            ({
              messages,
              currentActiveRoomId,
            }: {
